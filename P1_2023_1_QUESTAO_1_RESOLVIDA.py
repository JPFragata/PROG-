# -*- coding: utf-8 -*-
"""
Created on Wed Sep 25 16:12:03 2024
...
def exibeTotalRecebidoPorUmCPF(dPorPagador,CPF):
    pagamento=0
    for (pagador,dicInterno) in dPorPagador.items():
        for (recebedor,valor) in dicInterno.items():
            if recebedor == CPF:
                pagamento+=valor
    print(f'{CPF} recebeu no total R${pagamento},00')
...
def cria_dic_por_vantagem(dic_energia):
    dic={}
    for (energia,dicInterno) in dic_energia.items():
        lista_vantagens = dicInterno['vantagens']
        for vantagem in lista_vantagens:
            valor = dic.get(vantagem,[])
            dic[vantagem] = valor
            dic[vantagem].append(energia)
    return dic
